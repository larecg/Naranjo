...
import { getListOfModels, sendPrompt } from "./xaiService";
...
describe("XAIService", () => {
  ...
  describe("getListOfModels", () => {
    test("it should fetch and return grok models from the xAI API", async () => {
      const mockResponse = {
        data: [
          { id: "grok-2-latest" },
      ...
      expect(result).toEqual(["grok-2-latest", "grok-2-vision-latest"]);
  ...
  describe("sendPrompt", () => {
    ...
    test("it should send prompt and return the response content", async () => {
      ...
      expect(global.fetch).toHaveBeenCalledWith(
        ...
        expect.objectContaining({
          method: "POST",
          body: expect.stringContaining('"model":"grok-2-latest"'),
        })
      );
    });
    ...
    test("it should throw when the provider is not configured", async () => {
      ...
      await expect(sendPrompt(testParams)).rejects.toThrow(
        "xAI API Key is not configured or provider is disabled."
      );
    });
